...
export default function StudentsPage() {
  ...
  return (
    <div>
      ...
      {detailsModalOpen && selectedCard && <div onClick={() => setDetailsModalOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.48)', display: 'grid', placeItems: 'center', padding: 20, zIndex: 40 }}><div onClick={(e) => e.stopPropagation()} style={{ width: 'min(920px, 100%)', maxHeight: '88vh', overflowY: 'auto', background: '#fff', borderRadius: 24, border: '1px solid rgba(24,33,47,0.08)', boxShadow: '0 30px 80px rgba(15,23,42,0.18)', padding: 24, display: 'grid', gap: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}><div><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}><h3 style={{ fontSize: 26, marginBottom: 0 }}>{selectedCard.student.full_name}</h3><button type="button" title="Удалить связь" onClick={handleDeleteTutorStudent} style={{ minWidth: 42, width: 42, height: 42, padding: 0, borderRadius: 999, background: 'rgba(166,63,59,0.92)', boxShadow: 'none', fontSize: 18 }}>🗑</button></div><div style={mutedTextStyle}>{selectedStudentCards.length > 1 ? `${selectedStudentCards.length} предмета` : selectedCard.subject?.name ?? t.subjectMissing}</div></div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}><button type="button" title="Закрыть" onClick={() => setDetailsModalOpen(false)} style={{ minWidth: 42, width: 42, height: 42, padding: 0, borderRadius: 999, background: 'rgba(23,32,51,0.92)', boxShadow: 'none', fontSize: 22 }}>×</button></div></div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{[`${t.phone}: ${selectedCard.student.phone_number || '\u2014'}`, `${t.grade}: ${selectedCard.student.grade || '\u2014'}`, `Telegram ID: ${selectedCard.student.telegram_id || '\u2014'}`, `${t.contacts}: ${selectedCard.contacts.length}`].map((item) => <span key={item} style={{ padding: '6px 10px', borderRadius: 999, background: 'rgba(23,32,51,0.06)', color: '#324055', fontSize: 13 }}>{item}</span>)}</div><section style={{ ...panelStyle, padding: 14 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 10, flexWrap: 'wrap' }}><div style={{ fontWeight: 800, color: '#1f2a3b' }}>Краткая статистика</div><button type="button" title="Открыть полное портфолио" onClick={() => navigate(`/portfolio?student_id=${selectedCard.student.id}`)} style={{ background: '#2a6fdb', boxShadow: 'none', padding: '10px 14px' }}>Портфолио</button></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>{[['Проведено занятий', selectedConductedLessons.length], ['Оценка занятий', averageLessonGrade === null ? '—' : averageLessonGrade.toFixed(1)], ['Оценка ДЗ', averageAssignmentGrade === null ? '—' : averageAssignmentGrade.toFixed(1)], ['Выполнено ДЗ', `${completedAssignmentsCount}/${selectedStudentAssignments.length}`]].map(([label, value]) => <div key={label} style={{ padding: 12, borderRadius: 14, background: 'rgba(23,32,51,0.04)', border: '1px solid rgba(24,33,47,0.06)' }}><div style={{ ...mutedTextStyle, marginBottom: 6 }}>{label}</div><div style={{ color: '#1f2a3b', fontSize: 22, fontWeight: 900 }}>{value}</div></div>)}</div></section><section style={{ ...panelStyle, padding: 14 }}><div style={{ fontWeight: 800, color: '#1f2a3b', marginBottom: 10 }}>Предметы ученика</div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>{selectedStudentCards.map((card) => <button key={card.tutorStudent.id} type="button" onClick={() => setSelectedTutorStudentId(card.tutorStudent.id)} style={{ background: selectedCard.tutorStudent.id === card.tutorStudent.id ? '#d96f32' : 'rgba(23,32,51,0.08)', color: selectedCard.tutorStudent.id === card.tutorStudent.id ? '#fff' : '#324055', boxShadow: 'none' }}>{card.subject?.name ?? t.subjectMissing}</button>)}</div></section><section style={{ ...panelStyle, padding: 14 }}><div style={{ fontWeight: 800, color: '#1f2a3b', marginBottom: 10 }}>{'\u041e\u0441\u043d\u043e\u0432\u043d\u044b\u0435 \u043f\u0430\u0440\u0430\u043c\u0435\u0442\u0440\u044b'}</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10, marginBottom: 12 }}><label style={{ display: 'grid', gap: 6, color: '#556173', fontSize: 14 }}>{t.rate}<input type="number" value={detailRate} onChange={(e) => setDetailRate(e.target.value)} /></label><label style={{ display: 'grid', gap: 6, color: '#556173', fontSize: 14 }}>{t.status}<select value={detailStatus} onChange={(e) => setDetailStatus(e.target.value as TutorStudentStatus)}><option value="active">active</option><option value="paused">paused</option><option value="completed">completed</option></select></label><label style={{ display: 'grid', gap: 6, color: '#556173', fontSize: 14 }}>{t.grade}<input type="number" min={1} max={11} value={detailGrade} onChange={(e) => setDetailGrade(e.target.value)} placeholder="1-11" /></label></div><div style={{ marginBottom: 12 }}><div style={{ color: '#556173', fontSize: 14, marginBottom: 8 }}>Уровни обучения по предмету</div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>{tutorLevels.length === 0 ? <span style={mutedTextStyle}>Уровни ещё не созданы.</span> : tutorLevels.map((level) => { const active = detailLevelIds.includes(level.id); return <button key={level.id} type="button" onClick={() => setDetailLevelIds((current) => active ? current.filter((id) => id !== level.id) : [...current, level.id])} style={{ padding: '8px 10px', borderRadius: 999, background: active ? 'rgba(42,111,219,0.12)' : 'rgba(23,32,51,0.05)', color: active ? '#2a6fdb' : '#324055', border: active ? '1px solid rgba(42,111,219,0.24)' : '1px solid rgba(24,33,47,0.08)', boxShadow: 'none', fontWeight: 700, fontSize: 12 }}>{level.name}</button>; })}</div></div><button type="button" onClick={handleSaveTutorStudent} disabled={savingTutorStudent}>{savingTutorStudent ? '\u0421\u043e\u0445\u0440\u0430\u043d\u044f\u0435\u043c...' : '\u0421\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u044f'}</button></section><section style={{ ...panelStyle, padding: 14 }}><div style={{ fontWeight: 800, color: '#1f2a3b', marginBottom: 8 }}>{'\u041a\u043e\u043d\u0442\u0430\u043a\u0442\u043d\u044b\u0435 \u043b\u0438\u0446\u0430'}</div>{selectedCard.contacts.length === 0 ? <p style={{ ...mutedTextStyle, marginBottom: 12 }}>{'\u0423 \u044d\u0442\u043e\u0433\u043e \u0443\u0447\u0435\u043d\u0438\u043a\u0430 \u043f\u043e\u043a\u0430 \u043d\u0435\u0442 \u043f\u0440\u0438\u0432\u044f\u0437\u0430\u043d\u043d\u044b\u0445 \u043a\u043e\u043d\u0442\u0430\u043a\u0442\u043d\u044b\u0445 \u043b\u0438\u0446.'}</p> : <div style={{ display: 'grid', gap: 10, marginBottom: 12 }}>{selectedCard.contacts.map((item) => <div key={item.id} style={{ padding: 12, borderRadius: 14, background: 'rgba(23,32,51,0.03)', border: '1px solid rgba(24,33,47,0.08)' }}><div style={{ fontWeight: 700, color: '#243041', marginBottom: 4 }}>{item.contact.full_name}</div><div style={{ color: '#556173', fontSize: 14 }}>{`${t.phone}: ${item.contact.phone_number || '\u2014'} \u2022 Telegram ID: ${item.contact.telegram_id || '\u2014'}`}</div></div>)}</div>}<button type="button" onClick={() => navigate('/contacts')} style={{ background: 'rgba(23,32,51,0.92)', boxShadow: 'none' }}>{'\u041e\u0442\u043a\u0440\u044b\u0442\u044c \u043a\u043e\u043d\u0442\u0430\u043a\u0442\u043d\u0443\u044e \u043a\u043d\u0438\u0436\u043a\u0443'}</button></section></div></div>}
    </div>
  );
}
