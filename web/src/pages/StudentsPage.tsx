...
export default function StudentsPage() {
  ...
  const [sortMode, setSortMode] = useState<'name' | 'grade'>('name');
  ...
  return (
    <div style={{ display: 'grid', gridTemplateRows: 'auto auto auto minmax(0, 1fr)', gap: 16, height: '100%', minHeight: 0, overflow: 'hidden' }}>
      ...
      <section className="mentor-panel toolbar-panel" style={{ gridTemplateColumns: 'minmax(260px, 1.35fr) minmax(150px, 0.55fr) minmax(190px, 0.7fr) minmax(140px, 0.45fr) auto auto' }}>
            ...
            <select aria-label="Сортировка учеников" value={sortMode} onChange={(e) => setSortMode(e.target.value as 'name' | 'grade')}>
        ...
        <div onClick={() => setDetailsModalOpen(false)} className="modal-overlay">
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
                  <h3 style={{ fontSize: 26, marginBottom: 0 }}>{selectedCard.student.full_name}</h3>
                  <button
                    type="button"
                    title="Удалить связь"
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {[
                `${t.phone}: ${selectedCard.student.phone_number || '—'}`,
                `${t.grade}: ${selectedCard.student.grade || '—'}`,
                `Telegram ID: ${selectedCard.student.telegram_id || '—'}`,
                `Занимается с: ${formatRelationStartDate(selectedCard.tutorStudent.started_at)}`,
                `${t.contacts}: ${selectedCard.contacts.length}`,
              ].map((item) => (
                <span
                  key={item}
                  style={{
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                <section style={{ ...panelStyle, padding: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                    {selectedStudentCards.map((card) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setSelectedTutorStudentId(card.tutorStudent.id)}
                        style={{
                          background: selectedCard.tutorStudent.id === card.tutorStudent.id ? '#2AABEE' : 'rgba(23,32,51,0.08)',
                          color: selectedCard.tutorStudent.id === card.tutorStudent.id ? '#fff' : '#324055',
                          boxShadow: 'none',
                        }}
                      >
                        {card.subject?.name ?? t.subjectMissing}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 8, marginBottom: 10 }}>
                    ...
                    <label style={{ display: 'grid', gap: 6, color: '#556173', fontSize: 14 }}>
                      {t.status}
                      <select value={detailStatus} onChange={(event) => setDetailStatus(event.target.value as TutorStudentStatus)}>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ color: '#556173', fontSize: 14, marginBottom: 8 }}>Уровни обучения по предмету</div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {tutorLevels.length === 0 ? (
                        <span style={mutedTextStyle}>Уровни ещё не созданы.</span>
                      ) : (
                        tutorLevels.map((level) => {
                          const active = detailLevelIds.includes(level.id);
                          return (
                            <button
                              key={level.id}
                              type="button"
                              onClick={() =>
                                setDetailLevelIds((current) =>
                              ...
                              style={{
                                ...
                                border: active ? '1px solid rgba(42,171,238,0.24)' : '1px solid rgba(24,33,47,0.08)',
                                boxShadow: 'none',
                                fontWeight: 700,
                                fontSize: 12,
                              }}
                  ...
                  <button type="button" onClick={handleSaveTutorStudent} disabled={savingTutorStudent} className="modal-primary">
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                <section style={{ ...panelStyle, padding: 12 }}>
                  ...
                  {selectedCard.contacts.length === 0 ? (
                    <p style={{ ...mutedTextStyle, marginBottom: 12 }}>У этого ученика пока нет привязанных контактных лиц.</p>
                  ) : (
                    <div style={{ display: 'grid', gap: 8, marginBottom: 12 }}>
                      {selectedCard.contacts.map((item) => (
                        <div
                          key={item.id}
                          style={{ padding: 10, borderRadius: 14, background: 'rgba(23,32,51,0.03)', border: '1px solid rgba(24,33,47,0.08)' }}
                        >
                          <div style={{ fontWeight: 700, color: '#243041', marginBottom: 4 }}>{item.contact.full_name}</div>
                          <div style={{ color: '#556173', fontSize: 14 }}>
                            {`${t.phone}: ${item.contact.phone_number || '—'} • Telegram ID: ${item.contact.telegram_id || '—'}`}
                          </div>
                        </div>
                      ))}
                    </div>
                  )}
                  <button type="button" onClick={() => navigate('/contacts')} className="modal-secondary">
