...
export default function StudentsPage() {
  ...
  const [sortMode, setSortMode] = useState<'name' | 'grade' | 'balance'>('name');
  ...
  const formatShortLessonDate = (value: string) => {
  ...
  const getNextLessonForGroup = (group: StudentGroupData) => {
    const relationIds = new Set(group.cards.map((card) => card.tutorStudent.id));
    const now = Date.now();
    ...
    return lessons
      .filter(
        (lesson) =>
          ...
          relationIds.has(lesson.tutor_student_id) &&
          lesson.conduct_status === 'scheduled' &&
          !Number.isNaN(new Date(lesson.starts_at).getTime()) &&
          new Date(lesson.starts_at).getTime() >= now
      )
      .sort((a, b) => new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime())[0];
  };

  const getBalanceForGroup = (group: StudentGroupData) => {
    const relationIds = new Set(group.cards.map((card) => card.tutorStudent.id));
    return lessons
      .filter(
        (lesson) =>
  ...
  const sortGroups = (groups: StudentGroupData[]) =>
    [...groups].sort((a, b) => {
      if (sortMode === 'grade') {
        return (a.student.grade ?? 99) - (b.student.grade ?? 99) || a.student.full_name.localeCompare(b.student.full_name, 'ru-RU');
      }
      if (sortMode === 'balance') {
        return getBalanceForGroup(b) - getBalanceForGroup(a) || a.student.full_name.localeCompare(b.student.full_name, 'ru-RU');
      }
    ...
    return (
      <button key={group.student.id} type="button" onClick={() => { setSelectedTutorStudentId(primary.tutorStudent.id); setDetailsModalOpen(true); }} style={{ display: 'grid', gridTemplateRows: 'auto auto 1fr auto', minHeight: 188, gap: 14, padding: 18, textAlign: 'left', borderRadius: 22, background: '#fff', color: '#1f2a3b', border: '1px solid rgba(24,33,47,0.08)', boxShadow: 'var(--shadow-soft)' }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          ...
          <span style={{ width: 34, height: 34, borderRadius: 12, display: 'inline-grid', placeItems: 'center', background: 'rgba(23,32,51,0.04)', color: '#687486', fontWeight: 900 }}>↗</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'end' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ color: '#687486', fontSize: 13, marginBottom: 4 }}>Баланс</div>
            <div style={{ color: balance > 0 ? '#4CAF50' : '#1f2a3b', fontWeight: 900 }}>{balance > 0 ? `+${balance.toLocaleString('ru-RU')} ₽` : '0 ₽'}</div>
          </div>
        </div>
      </button>
    );
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <section className="mentor-panel toolbar-panel" style={{ gridTemplateColumns: 'minmax(260px, 1.35fr) minmax(150px, 0.55fr) minmax(190px, 0.7fr) minmax(140px, 0.45fr) auto auto' }}>
            ...
            <select aria-label="Сортировка учеников" value={sortMode} onChange={(e) => setSortMode(e.target.value as 'name' | 'grade' | 'balance')}>
              ...
              <div className="modal-form-grid">
                ...
                <div className="modal-actions">
                  <button type="button" onClick={handleCreateStudent} disabled={creatingStudent} className="modal-primary">
                    {creatingStudent ? 'Создаём...' : t.create}
                  </button>
                </div>
              </div>
            )}
